...
export default defineEventHandler(async (event) => {
  ...
  await rateLimitByUser(event, 'report-annotation', { max: 10, windowSec: 86400 })
  ...
  const annotation = await db
    .select({ id: schema.annotations.id, restroomId: schema.annotations.restroomId, authorId: schema.annotations.authorId })
  ...
  if (annotation.authorId === user.id) {
    throw createError({ statusCode: 400, statusMessage: 'You cannot report your own annotation.' })
  }
