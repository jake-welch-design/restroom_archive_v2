...
export default defineEventHandler(async (event) => {
  ...
  const { imageData } = await readValidatedBody(event, z.object({
    imageData: z.string().regex(IMAGE_DATA_RE, 'Invalid image data URI').max(MAX_IMAGE_BASE64_CHARS, 'Image too large'),
  }).parse)
  ...
  if (!isValidImageMagic(buffer)) {
    throw createError({ statusCode: 422, statusMessage: 'File is not a valid image' })
  }
