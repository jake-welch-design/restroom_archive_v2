...
export default defineEventHandler(async (event) => {
  ...
  if (!meta.success) {
    throw createError({ statusCode: 422, statusMessage: meta.error.issues[0]?.message ?? 'Invalid fields' })
  }

  const { name, location, isoDate, lat, lng, description, descriptors } = meta.data
  let descriptorTags: string[] = []
  if (descriptors) {
    try {
      const parsed = JSON.parse(descriptors)
      if (Array.isArray(parsed)) descriptorTags = parsed.filter((t): t is string => typeof t === 'string' && t.length <= 40)
    }
  ...
  const existing = await db
    ...
    .get()
  if (existing) throw createError({ statusCode: 409, statusMessage: 'A restroom with this name/location/date already exists' })
