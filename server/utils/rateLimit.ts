...
export async function rateLimitByUser(event: H3Event, action: string, opts: RateLimitOptions) {
...
export async function rateLimitByIp(event: H3Event, action: string, opts: RateLimitOptions) {
  const ip = getHeader(event, 'CF-Connecting-IP') ?? getHeader(event, 'X-Forwarded-For') ?? 'unknown'
  await check(event, `i:${ip}:${action}`, opts)
}

async function check(event: H3Event, key: string, { max, windowSec }: RateLimitOptions) {
  ...
  if ((row?.count ?? 0) > max) {
    const retryAfter = Math.max(1, (win + 1) * windowSec - Math.floor(Date.now() / 1000))
  ...
  if (Math.random() < 0.01) {
    await db.prepare('DELETE FROM rate_limits WHERE window < ?').bind(win - 10).run()
  }
}
